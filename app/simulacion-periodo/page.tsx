  ...
  return (
    <div className="main-wrapper">
      <div className="card map-card" style={{ padding: 0, overflow: 'visible', minHeight: '100vh', display: 'flex', flexDirection: 'column', width: '100%', margin: '0', borderRadius: 0, position: 'relative' }}>
        ...
        <div style={{ position: 'absolute', top: '12px', left: '0', right: '0', display: 'flex', justifyContent: 'center', pointerEvents: 'none', zIndex: 999999 }}>
          <div style={{ pointerEvents: 'auto', padding: '8px 12px' }}>
            <div className="card" style={{ display: 'flex', gap: '14px', marginBottom: '0', padding: '10px 12px', alignItems: 'center', justifyContent: 'space-between' }}>
              <h1 style={{ marginBottom: '0', fontSize: '20px', color: 'var(--text-primary)', whiteSpace: 'nowrap' }}>📅 Simulación de Período</h1>
              <div style={{ height: '20px', width: '1px', backgroundColor: 'var(--border-color)' }}></div>
              <div style={{ display: 'flex', gap: '14px', alignItems: 'center' }}>
                <div style={{ minWidth: 0 }}>
                  <small style={{ color: 'var(--text-secondary)', fontWeight: 700, textTransform: 'uppercase', fontSize: '10px' }}>Período Simulado</small>
                  <div style={{ fontSize: '14px', fontWeight: 700, color: 'var(--accent-blue)', marginTop: '2px' }}>{totalDays} días</div>
                </div>
                <div style={{ minWidth: 0 }}>
                  <small style={{ color: 'var(--text-secondary)', fontWeight: 700, textTransform: 'uppercase', fontSize: '10px' }}>Día Actual</small>
                  <div style={{ fontSize: '14px', fontWeight: 700, color: 'var(--accent-blue)', marginTop: '2px' }}>{simulationDay} / {totalDays}</div>
                </div>
                <div style={{ minWidth: 0 }}>
                  <small style={{ color: 'var(--text-secondary)', fontWeight: 700, textTransform: 'uppercase', fontSize: '10px' }}>Progreso</small>
                  <div style={{ fontSize: '14px', fontWeight: 700, color: '#f97316', marginTop: '2px' }}>{Math.round(progress)}%</div>
                </div>
              </div>
              <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexShrink: 0 }}>
                <button onClick={() => setSimulationDay(Math.max(1, simulationDay - 1))} style={{ padding: '8px 10px', fontSize: '12px', backgroundColor: 'var(--bg-tertiary)', border: '1px solid var(--border-color)', borderRadius: '6px', cursor: 'pointer', color: 'var(--text-primary)' }}>⏮️</button>
                <button onClick={() => setSimulationDay(simulationDay + 1)} style={{ padding: '8px 10px', fontSize: '12px', backgroundColor: 'var(--accent-blue)', border: 'none', borderRadius: '6px', cursor: 'pointer', color: 'white' }}>▶️ Siguiente Día</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ position: 'absolute', top: '12px', right: '12px', width: 'auto', maxWidth: '350px', backgroundColor: 'rgba(255, 255, 255, 0.95)', padding: '16px', borderRadius: '12px', boxShadow: '0 4px 20px rgba(0, 0, 0, 0.15)', pointerEvents: 'auto', zIndex: 999998 }}>
          <div style={{ fontSize: '11px', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '8px', textTransform: 'uppercase' }}>Progreso de Simulación</div>
          <div style={{ width: '100%', height: '8px', backgroundColor: 'var(--border-color)', borderRadius: '4px', overflow: 'hidden', marginBottom: '8px' }}>
            <div style={{ height: '100%', backgroundColor: 'var(--accent-blue)', width: `${progress}%`, transition: 'width 0.3s ease' }}></div>
          </div>
          <div style={{ fontSize: '13px', fontWeight: 700, color: 'var(--text-primary)' }}>Día {simulationDay} de {totalDays}</div>
        </div>
      </div>
      ...
      <div id="statsSection" style={{ paddingTop: '40px', paddingBottom: '40px', backgroundColor: 'white' }}>
        <div className="container">
          <h2 style={{ fontSize: '24px', fontWeight: 700, marginBottom: '24px', color: 'var(--text-primary)' }}>📊 Resultados por Día</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
            {Array.from({ length: totalDays }).map((_, day) => (
              <div key={day} style={{ padding: '16px', backgroundColor: day < simulationDay ? '#f0fdf4' : 'var(--bg-tertiary)', border: `1px solid ${day < simulationDay ? '#22c55e' : 'var(--border-color)'}`, borderRadius: '8px' }}>
                <div style={{ fontSize: '12px', fontWeight: 600, color: day < simulationDay ? '#22c55e' : 'var(--text-secondary)', textTransform: 'uppercase', marginBottom: '12px' }}>
                  {day < simulationDay ? '✓ Completado' : day === simulationDay - 1 ? 'En Progreso' : 'Pendiente'}
                </div>
                <div style={{ fontSize: '18px', fontWeight: 700, color: 'var(--text-primary)', marginBottom: '8px' }}>Día {day + 1}</div>
                <div style={{ fontSize: '13px', color: 'var(--text-secondary)', marginBottom: '8px' }}>
                  <div>Maletas: {170 + day * 20}</div>
                  <div>Vuelos: {4 + day}</div>
                  <div>Entregas: {150 + day * 18}</div>
                </div>
              </div>
            ))}
          </div>

          <h2 style={{ fontSize: '24px', fontWeight: 700, marginBottom: '24px', marginTop: '40px', color: 'var(--text-primary)' }}>📈 Estadísticas Acumuladas</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '12px' }}>
            <div className="stat-card">
              <h3 style={{ fontSize: '13px', margin: '0 0 8px 0' }}>Total Maletas Procesadas</h3>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flex: 1 }}>
                <div style={{ fontSize: '28px', fontWeight: 700, color: 'var(--accent-blue)' }}>{900 + simulationDay * 20}</div>
              </div>
            </div>

            <div className="stat-card">
              <h3 style={{ fontSize: '13px', margin: '0 0 8px 0' }}>Entregas Exitosas</h3>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flex: 1 }}>
                <div style={{ fontSize: '28px', fontWeight: 700, color: '#22c55e' }}>{850 + simulationDay * 18}</div>
              </div>
            </div>

            <div className="stat-card">
              <h3 style={{ fontSize: '13px', margin: '0 0 8px 0' }}>Incumplidas</h3>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flex: 1 }}>
                <div style={{ fontSize: '28px', fontWeight: 700, color: '#ef4444' }}>{50 + simulationDay * 2}</div>
              </div>
            </div>

            <div className="stat-card">
              <h3 style={{ fontSize: '13px', margin: '0 0 8px 0' }}>% Puntualidad</h3>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flex: 1 }}>
                <div style={{ fontSize: '28px', fontWeight: 700, color: 'var(--accent-blue)' }}>94%</div>
              </div>
            </div>
          </div>
            ...
            <div className="card">
              ...
              <div style={{ maxHeight: '300px', overflowY: 'auto' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  <span style={{ width: '8px', height: '8px', borderRadius: '50%', backgroundColor: '#22c55e', flexShrink: 0 }}></span>
                  <span style={{ fontSize: '14px', color: 'var(--text-primary)' }}>Simulación iniciada: Período {totalDays} días</span>
                </div>
              </div>
            </div>
            ...
            <div className="card">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <div style={{ padding: '12px', backgroundColor: 'var(--bg-tertiary)', borderRadius: '6px' }}>
                  ...
                  <div style={{ fontSize: '24px', fontWeight: 700, color: '#22c55e', marginTop: '8px' }}>94%</div>
                  <small style={{ color: 'var(--text-muted)', fontSize: '11px', display: 'block', marginTop: '4px' }}>↑ 2.1% respecto a ayer</small>
                </div>

                <div style={{ padding: '12px', backgroundColor: 'var(--bg-tertiary)', borderRadius: '6px' }}>
                  <small style={{ color: 'var(--text-secondary)', textTransform: 'uppercase', fontWeight: 600, fontSize: '11px' }}>Ocupación Promedio</small>
                  <div style={{ fontSize: '24px', fontWeight: 700, color: '#f97316', marginTop: '8px' }}>72%</div>
                  <small style={{ color: 'var(--text-muted)', fontSize: '11px', display: 'block', marginTop: '4px' }}>En 7 almacenes activos</small>
                </div>
                ...
                <div style={{ padding: '12px', backgroundColor: 'var(--bg-tertiary)', borderRadius: '6px' }}>
                  ...
                  <small style={{ color: 'var(--text-muted)', fontSize: '11px', display: 'block', marginTop: '4px' }}>Desde origen a destino</small>
                </div>

                <div style={{ padding: '12px', backgroundColor: 'var(--bg-tertiary)', borderRadius: '6px' }}>
                  <small style={{ color: 'var(--text-secondary)', textTransform: 'uppercase', fontWeight: 600, fontSize: '11px' }}>Maletas Transitando</small>
                  <div style={{ fontSize: '24px', fontWeight: 700, color: 'var(--accent-blue)', marginTop: '8px' }}>{900 + simulationDay * 50}</div>
